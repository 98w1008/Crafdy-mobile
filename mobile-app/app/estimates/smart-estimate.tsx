...
type EstimateStep = 'client_selection' | 'scope_definition' | 'unified_upload' | 'ai_analysis' | 'review' | 'finalize'
...
export default function SmartEstimateScreen() {
    ...
    <View style={styles.stepContainer}>
      <Card variant="elevated" style={styles.scopeCard}>
        ...
        <StyledButton
          ...
          size="lg"
          elevated={true}
          onPress={() => setCurrentStep('unified_upload')}
  ...
  // 統合アップロード画面
  const renderUnifiedUpload = () => (
    <View style={styles.stepContainer}>
      <Card variant="premium" style={styles.uploadCard}>
        <StyledText variant="subtitle" weight="semibold" style={styles.sectionTitle}>
          📁 プロジェクト資料をアップロード
        </StyledText>
        <StyledText variant="body" color="secondary" style={{ marginBottom: spacing[4] }}>
          図面・仕様書・写真などをドラッグ&ドロップで追加してください。
          AIが自動的に内容を解析して見積に反映します。
        </StyledText>

        <DocumentUploader
          onFilesChange={(files) => {
            setProjectScope(prev => ({ ...prev, uploadedFiles: files }))
          }}
          maxFiles={20}
          allowedDocTypes={['drawing', 'spec', 'photo', 'receipt', 'contract']}
          title="スマート統合アップロード"
          description="複数ファイルを一度に追加できます"
          uploadMode="batch"
          showPreview={true}
        />
        ...
        {projectScope.uploadedFiles && projectScope.uploadedFiles.length > 0 && (
          <View style={styles.uploadSummary}>
            <StyledText variant="body" weight="medium" style={{ marginBottom: spacing[2] }}>
              📊 アップロード済み ({projectScope.uploadedFiles.length}件)
            </StyledText>
            <View style={styles.fileTypeBreakdown}>
              {[
                { type: 'drawing', label: '図面', icon: '📐' },
                { type: 'spec', label: '仕様書', icon: '📋' },
                { type: 'photo', label: '写真', icon: '📷' },
                { type: 'receipt', label: '見積・領収書', icon: '🧾' }
              ].map(({ type, label, icon }) => {
                const count = projectScope.uploadedFiles?.filter(f => f.docType === type).length || 0
                return count > 0 ? (
                  <Chip key={type} mode="outlined" compact style={styles.fileTypeChip}>
                    {icon} {label}: {count}件
                  </Chip>
                ) : null
              })}
        ...
        <View style={styles.uploadActions}>
          ...
          />
          <StyledButton
            title={projectScope.uploadedFiles?.length ? "🤖 AI分析開始" : "ファイルなしで続行"}
            variant="primary"
            size="lg"
            elevated={true}
          ...
          <View style={styles.analysisSteps}>
            {projectScope.uploadedFiles && projectScope.uploadedFiles.length > 0 && (
              <View style={[styles.analysisStep, { opacity: progress >= 0.15 ? 1 : 0.5 }]}>
                <StyledText variant="body" weight="medium">📄 ドキュメント解析</StyledText>
              </View>
            )}
            <View style={[styles.analysisStep, { opacity: progress >= 0.35 ? 1 : 0.5 }]}>
              <StyledText variant="body" weight="medium">📊 市場データ分析</StyledText>
            </View>
            <View style={[styles.analysisStep, { opacity: progress >= 0.55 ? 1 : 0.5 }]}>
              <StyledText variant="body" weight="medium">👤 顧客特性分析</StyledText>
            </View>
            <View style={[styles.analysisStep, { opacity: progress >= 0.75 ? 1 : 0.5 }]}>
              <StyledText variant="body" weight="medium">💰 価格最適化</StyledText>
            </View>
            <View style={[styles.analysisStep, { opacity: progress >= 0.9 ? 1 : 0.5 }]}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        {currentStep === 'unified_upload' && renderUnifiedUpload()}
