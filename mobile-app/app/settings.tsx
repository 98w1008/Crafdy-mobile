...
export default function SettingsScreen() {
  ...
  const { isDark, themeMode: ctxThemeMode, setThemeMode } = useTheme()
  ...
  // テーマ設定の読み込み
  useEffect(() => {
    // ThemeProviderの現在値をローカル表示状態に反映
    setThemeModeState(ctxThemeMode as ThemeMode)
  }, [ctxThemeMode])

  const saveThemeSettings = async (mode: ThemeMode) => {
    try {
      // ThemeProviderのAPIで保存＋反映（内部でAsyncStorage保存済み）
  ...
  const renderHeader = () => (
    <Surface style={[styles.header, { backgroundColor: colors.surface }]}>
      ...
      />
      <Text variant="headlineSmall" style={[styles.headerTitle, { color: colors.text.primary }]}>設定</Text>
      <View style={{ width: 48 }} />
    </Surface>
  )
  ...
  const renderProfileSection = () => (
    <Surface style={[styles.section, { backgroundColor: colors.surface }]}>
      <Text variant="titleMedium" style={[styles.sectionTitle, { color: colors.text.primary }]}>プロフィール</Text>
  ...
  const renderThemeSection = () => (
    <Surface style={[styles.section, { backgroundColor: colors.surface }]}>
      <Text variant="titleMedium" style={[styles.sectionTitle, { color: colors.text.primary }]}>外観</Text>
  ...
  useEffect(() => { (async()=>{ try { const v = await AsyncStorage.getItem('chat_only_mode'); setChatOnly(v==='1') } catch {} })() }, [])
  const toggleChatOnly = async (v: boolean) => { try { await AsyncStorage.setItem('chat_only_mode', v?'1':'0'); setChatOnly(v) } catch {} }

  const renderAppSettingsSection = () => (
    <Surface style={[styles.section, { backgroundColor: colors.surface }]}>
      <Text variant="titleMedium" style={[styles.sectionTitle, { color: colors.text.primary }]}>アプリ設定</Text>
      ...
      <List.Item
        title="チャットだけで操作する"
        description="作成/編集画面を使わず、チャットに集約"
        left={(props) => <List.Icon {...props} icon="chat" />}
        right={() => (
          <Switch
            value={chatOnly}
            onValueChange={toggleChatOnly}
            color="#007AFF"
          />
        )}
      />
  ...
  const renderSupportSection = () => (
    <Surface style={[styles.section, { backgroundColor: colors.surface }]}>
      <Text variant="titleMedium" style={[styles.sectionTitle, { color: colors.text.primary }]}>サポート・情報</Text>
  ...
  const renderAccountSection = () => (
    <Surface style={[styles.section, { backgroundColor: colors.surface }]}>
      <Text variant="titleMedium" style={[styles.sectionTitle, { color: colors.text.primary }]}>アカウント</Text>
