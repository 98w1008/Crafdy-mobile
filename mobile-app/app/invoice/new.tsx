...
export default function NewInvoiceScreen() {
  ...
  const [invoiceDate, setInvoiceDate] = useState(formatIsoDate(nowJp()))
  ...
  const handleSubmit = async () => {
    ...
    if (!clientName.trim()) {
      validationErrors.push('客户名を入力してください');
    }
    
    if (!projectName.trim()) {
      validationErrors.push('プロジェクト名を入力してください');
    }
    
    if (!parseJpDate(invoiceDate).isValid()) {
      validationErrors.push('有効な請求日を入力してください');
    }
    
    if (dueDate && !parseJpDate(dueDate).isValid()) {
      validationErrors.push('有効な支払期限を入力してください');
    }
    
    // 支払期限が請求日より後かチェック
    if (dueDate) {
      const invoiceDayjs = parseJpDate(invoiceDate);
      const dueDayjs = parseJpDate(dueDate);
      if (invoiceDayjs.isValid() && dueDayjs.isValid() && dueDayjs.isSameOrBefore(invoiceDayjs, 'day')) {
        validationErrors.push('支払期限は請求日より後の日付である必要があります');
      }
    }
    ...
    try {
      ...
      Alert.alert('エラー', '作成に失敗しました。もう一度お試しください。')
    } finally {
