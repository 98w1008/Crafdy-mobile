...
export default function EditInvoiceScreen() {
  ...
  const recalculateDueDate = async () => {
    ...
    try {
      // 発行日の妥当性チェック
      const parsedDate = parseJpDate(formData.issued_date);
      ...
      {activeTab === 'details' ? (
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.row}>
              <View style={styles.halfWidth}>
                <InvoiceField
                  label="発行日"
                  value={editMode ? formData.issued_date || '' : formatDateForDisplay(invoice.issued_date)}
                  editable={editMode}
              ...
              <View style={styles.halfWidth}>
                <InvoiceField
                  label="支払期日"
                  value={editMode ? formData.due_date || '' : formatDateForDisplay(invoice.due_date)}
                  editable={editMode}
          ...
          <View style={styles.section}>
            ...
            <InvoiceField
              label="作成日時"
              value={formatDateTimeForDisplay(invoice.created_at)}
              editable={false}
            />

            <InvoiceField
              label="更新日時"
              value={formatDateTimeForDisplay(invoice.updated_at)}
              editable={false}
            />
          </View>
        </ScrollView>
...
// ユーティリティ関数（dayjs ベース、Invalid Date 対応）
...
function formatDateTimeForDisplay(dateTimeString: string): string {
  ...
  const parsed = parseJpDate(dateTimeString);
  if (!parsed.isValid()) {
    console.warn('Invalid datetime in formatDateTimeForDisplay:', dateTimeString);
    return '無効な日時';
  }
