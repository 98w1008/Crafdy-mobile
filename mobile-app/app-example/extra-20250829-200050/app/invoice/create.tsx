...
export default function CreateInvoiceScreen() {
  ...
  const initializeWizard = async () => {
    try {
      ...
      if (settings) {
        ...
        // 支払期日を自動計算
        const calculation = await calculateDueDate(wizardState.formData.issued_date);
        setDueDateCalculation(calculation);
  ...
  const recalculateDueDate = async (issuedDate: string) => {
    try {
      const calculation = await calculateDueDate(issuedDate);
      setDueDateCalculation(calculation);
      updateFormData({ due_date: calculation.calculated_date });
    } catch (error) {
    ...
    <View style={styles.stepContainer}>
      ...
      >
        ...
        <View style={styles.dateValueContainer}>
          <Text style={styles.dateValue}>{formatDate(formData.issued_date)}</Text>
      ...
      >
        ...
        <View style={styles.dateValueContainer}>
          <Text style={styles.dateValue}>{formatDate(formData.due_date)}</Text>
          <Ionicons name="calendar-outline" size={20} color={Colors.text.secondary} />
        </View>
      </TouchableOpacity>
  ...
  return (
    <View style={styles.stepContainer}>
      ...
      <View style={styles.confirmationContainer}>
        ...
        <ConfirmationItem label="発行日" value={formatDate(formData.issued_date)} />
        <ConfirmationItem label="支払期日" value={formatDate(formData.due_date)} />
