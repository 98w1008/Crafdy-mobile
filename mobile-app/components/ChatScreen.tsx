  ...
  const renderMessage = ({ item }: { item: ChatMessage }) => {
    ...
    return (
      <View style={[
        ...
        isMyMessage
          ? [{ backgroundColor: colors.primary.DEFAULT }, styles.myMessage]
          : [{ backgroundColor: colors.surface }, styles.otherMessage]
      ]}>
        {!isMyMessage && (
          <Text style={[styles.userName, { color: colors.text.secondary }]}>{userName}</Text>
        )}
        ...
        {user ? (
          <EditableContent
            ...
            contentStyle={[
              styles.messageText,
              { color: isMyMessage ? colors.primary.contrastText || '#FFFFFF' : colors.text.primary }
            ]}
          />
        ) : (
          <Text style={[
        ...
        {item.photo_urls && item.photo_urls.length > 0 && (
          <View style={[styles.photoContainer, { backgroundColor: colors.background.secondary }]}>
            <Text style={[styles.photoText, { color: colors.text.secondary }]}>📷 写真 {item.photo_urls.length}枚</Text>
          </View>
        )}
        
        <Text style={[
          styles.timestamp,
          { color: isMyMessage ? 'rgba(255,255,255,0.8)' : colors.text.secondary }
        ]}>
    ...
    >
      ...
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        <TouchableOpacity 
          ...
          onPress={() => router.back()}
        >
          <Text style={[styles.backButtonText, { color: colors.primary.DEFAULT }]}>←</Text>
        </TouchableOpacity>
